#!/usr/bin/env python3
"""
Run Experiments with OpenRouter API (Gemma 4 26B Free)

This script runs QualSynth experiments using OpenRouter's free Gemma 4 26B model
...
def calculate_openrouter_iterations(
    dataset_name: str,
    batch_size: int = 20,  # Match qualsynth config default
...
def run_openrouter_experiments(
    ...
    Args:
        ...
        batch_size: Samples per iteration (default 20 from config)
    ...
    # Keep model conditions in caller-selected, non-overlapping roots.
    output_dir = (
        Path(output_root).expanduser().resolve()
        if output_root
        else project_root / 'results' / 'openrouter'
    )
    ...
    print(f"Batch size: {batch_size} samples/iteration (matches qualsynth config)")
    ...
    for idx, exp in enumerate(experiments, 1):
        ...
        try:
            ...
            # Run the experiment - keep config defaults unless the caller overrides them.
...
"""
    ...
    parser.add_argument(
        ...
        help=f'OpenRouter model slug (default: {OPENROUTER_MODEL})'
    )
    parser.add_argument(
        '--output-root',
        default=None,
        help='Explicit result root; required to keep comparative model conditions separate'
    )
    parser.add_argument(
        '--datasets', 
        nargs='+', 
    ...
    parser.add_argument(
        ...
        type=int, 
        default=20,
        help='Samples per iteration (default: 20 from qualsynth config)'
    )
    parser.add_argument(
        '--rate-limit-delay', 
        type=float, 
        default=None,
        help='Seconds between API calls (default: auto based on tier - 1.0s paid, 3.0s free)'
    )
    parser.add_argument(
        '--no-rate-limit', 
        action='store_true',
