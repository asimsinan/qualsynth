...
# Estimated time:
#   - Setup: 15-30 minutes
#   - Full experiments: ~80 hours (can be interrupted and resumed)
#
...
setup_virtual_environment() {
    ...
    pip install -e "$SCRIPT_DIR/qualsyn-1.0.0" -q
...
run_dry_run() {
    ...
    echo "   Would create venv at: $SCRIPT_DIR/venv"
    echo "   Would install packages:"
    if [[ -f "$SCRIPT_DIR/requirements.txt" ]]; then
        grep -v "^#" "$SCRIPT_DIR/requirements.txt" | grep -v "^$" | head -8 | sed 's/^/     - /'
        echo "     ... (and more)"
    fi
    echo ""
...
echo "  python replication.py --quick"
echo ""
echo "To run full experiments:"
echo "  python replication.py --full"
echo ""
