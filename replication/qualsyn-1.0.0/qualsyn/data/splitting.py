...
def decode_features(X: pd.DataFrame, preprocessor) -> pd.DataFrame:
    ...
    if hasattr(preprocessor, 'numerical_features') and preprocessor.numerical_features:
        X_decoded[preprocessor.numerical_features] = preprocessor.scaler.inverse_transform(
            X[preprocessor.numerical_features]
        )
    ...
    if hasattr(preprocessor, 'label_encoders'):
        for col, encoder in preprocessor.label_encoders.items():
            if col in X_decoded.columns:
                # Get encoded values and clip to valid range
                encoded_vals = X[col].astype(int).values.copy()
                n_classes = len(encoder.classes_)
                
                # Handle out-of-range values (e.g., -1 or values >= n_classes)
                # Clip to valid range [0, n_classes-1]
                invalid_mask = (encoded_vals < 0) | (encoded_vals >= n_classes)
                if invalid_mask.any():
                    # Replace invalid values with 0 (first category)
...
def binarize_sensitive_features(
    ...
    This function creates binary indicators (0/1) for protected groups BEFORE
    normalization, so they can be used for fairness evaluation.
    ...
    Example:
        For age with protected_group = "age >= 60":
        - Creates column 'age_protected' with 1 if age >= 60, else 0
        
        For marital with protected_group = "divorced":
        - Creates column 'marital_protected' with 1 if marital == "divorced", else 0
    """
...
def create_splits_with_preprocessor(
    ...
    This enables:
    - Loading raw data for LLM generation
    - Encoding generated data back to normalized format for training
    ...
    for seed in seeds:
        ...
        split_data = {
            ...
            'y_test': y_test,
            'seed': seed,
            'preprocessor': preprocessor  # CRITICAL: Save preprocessor for decode/encode
        }
