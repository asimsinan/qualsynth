...
class DatasetPreprocessor:
    ...
    def _preprocess_german_credit(self, df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.Series]:
        ...
        # Target: class (1=good, 2=bad) -> convert to binary (0=bad/minority, 1=good/majority)
        # We invert so that 1 is minority (bad credit) to match other datasets
        target = (df['class'] == 2).astype(int)  # 1 = bad credit (minority), 0 = good credit (majority)
        ...
        # Encode categorical features (they are strings like 'A11', 'A12', etc.)
