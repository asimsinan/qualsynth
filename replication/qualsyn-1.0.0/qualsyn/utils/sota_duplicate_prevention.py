"""
SOTA Duplicate Prevention System for LLM-Generated Synthetic Data

Combines multiple state-of-the-art approaches:
1. Semantic Deduplication (SemDeDup) - embedding-based similarity
2. Contrastive Sampling - diversity-aware generation
3. Memory-Augmented Generation - track generated samples
4. Real-time Filtering - prevent duplicates during generation
"""
...
class SOTADuplicatePrevention:
    """
    State-of-the-art duplicate prevention system combining multiple approaches.
    ...
    5. Sliding window memory management (IMPROVEMENT #3)
    """
    ...
    def __init__(
        ...
        """
        Initialize SOTA duplicate prevention system.
        ...
        Args:
            ...
            max_memory_size: Maximum number of samples to keep in memory (IMPROVEMENT #3)
        ...
        if self.verbose:
            print("🔬 SOTA Duplicate Prevention System Initialized")
    ...
    def fit(self, X_train: pd.DataFrame, categorical_features: Optional[List[str]] = None, 
        ...
        # CRITICAL FIX: Add training samples to memory so generated samples
        # are compared against BOTH original training data AND other generated samples
        if add_to_memory:
            if self.verbose:
    ...
    def filter_duplicates(
        ...
        """
        Filter duplicates using multiple SOTA methods.
        ...
        if self.verbose:
            print(f"\n🔬 SOTA Duplicate Prevention: Processing {original_count} samples...")
        ...
        for i, sample in enumerate(samples):
            ...
            if not is_duplicate:
                ...
                # Update memory with bounded management (IMPROVEMENT #3)
                # Reuse computed hash and embedding if available
                self._add_to_memory(
                    sample,
    ...
    def _add_to_memory(
        ...
        """
        Add sample to memory with bounded management (IMPROVEMENT #3).
        
        Automatically prunes memory when it exceeds max_memory_size.
        
        Args:
            sample: Sample dictionary to add
            sample_hash: Pre-computed hash (optional)
            sample_embedding: Pre-computed embedding (optional)
        """
        # Add to hash set
        if self.enable_hash_dedup:
        ...
        # IMPROVEMENT #3: Prune memory if it exceeds max size
        if len(self.seen_samples) > self.max_memory_size:
            self._prune_memory()
    
    def _prune_memory(self):
        """
        Prune memory based on configured strategy (IMPROVEMENT #3).
