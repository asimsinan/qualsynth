"""
SOTA Adaptive Validator with Diversity-Preserving Validation

This validator implements state-of-the-art techniques to preserve diversity
...
class AdaptiveValidator:
    ...
    def _validate_quality_adaptive(
        ...
        # Combine masks
        combined_mask = statistical_mask & fairness_mask
        
        stats = {
            'statistical_valid': statistical_mask.sum(),
            'fairness_valid': fairness_mask.sum(),
            'combined_valid': combined_mask.sum(),
            'statistical_validation_enabled': self.enable_statistical_validation
        }
        
        return X_generated[combined_mask].copy(), y_generated[combined_mask].copy(), stats
    ...
    def _validate_statistical_adaptive(
        ...
        for col in X_generated.columns:
            ...
            if X_train[col].dtype in [np.float64, np.float32, np.int64, np.int32]:
                ...
                range_mask = (gen_col >= col_min) & (gen_col <= col_max)
                mask &= range_mask
