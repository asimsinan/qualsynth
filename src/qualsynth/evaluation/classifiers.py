...
class ClassifierPipeline:
    ...
    VALID_IMBALANCE_POLICIES = {"balanced", "none"}

    def __init__(self, random_state: int = 42, imbalance_policy: str = "balanced"):
        """
        Initialize classifier pipeline.
        
        Args:
            random_state: Random seed for reproducibility
            imbalance_policy: ``balanced`` applies the benchmark's existing
                class weighting; ``none`` leaves every classifier unweighted.
        """
        normalized_policy = str(imbalance_policy).strip().lower()
        if normalized_policy not in self.VALID_IMBALANCE_POLICIES:
            allowed = ", ".join(sorted(self.VALID_IMBALANCE_POLICIES))
            raise ValueError(
                f"Unknown imbalance_policy={imbalance_policy!r}. Expected one of: {allowed}."
            )
        self.random_state = random_state
        self.imbalance_policy = normalized_policy
        self.classifiers = {}
        self.trained_models = {}
    ...
    def get_default_classifiers(self) -> Dict[str, BaseEstimator]:
        ...
        class_weight = "balanced" if self.imbalance_policy == "balanced" else None
        classifiers = {
    ...
    def train(
        ...
        # Adjust XGBoost only under the explicit balanced policy. Under ``none``
        # it remains at 1.0, matching an unweighted binary objective.
        if self.imbalance_policy == "balanced" and 'XGBoost' in classifiers:
    ...
    def get_imbalance_policy_metadata(self) -> Dict[str, Any]:
        """Return the resolved classifier weighting policy for run provenance."""
        classifiers = self.classifiers or self.get_default_classifiers()
        return {
            "policy": self.imbalance_policy,
            "random_forest_class_weight": classifiers["RandomForest"].get_params().get(
                "class_weight"
            ),
            "logistic_regression_class_weight": classifiers["LogisticRegression"].get_params().get(
                "class_weight"
            ),
            "xgboost_scale_pos_weight": classifiers["XGBoost"].get_params().get(
                "scale_pos_weight"
            ),
        }
