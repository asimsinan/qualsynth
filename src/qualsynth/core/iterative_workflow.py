...
This module orchestrates the active quality-and-diversity workflow:
1. Dataset Profiler -> analyze characteristics
2. Neutral objective setup -> disable fairness targets in the live path
3. Schema Profiler -> extract constraints
4. Diversity Planner -> identify sparse regions
5. ITERATION LOOP:
   a. Prompt Builder -> create anchor-centric prompts
   b. Counterfactual Generator -> generate samples
   c. Validator -> validate samples
   d. Multi-Objective Optimizer -> select best samples
   e. Repeat until convergence or the target sample count is reached
"""
...
try:
    ...
    from ..utils.llm_config import resolve_lmstudio_model
except ImportError:
    ...
    from src.qualsynth.utils.llm_config import resolve_lmstudio_model
...
class WorkflowConfig:
    ...
    # Legacy fairness parameters (disabled in the active workflow path)
    ...
    dynamic_few_shot: bool = True
    few_shot_selection_strategy: str = "mixed"  # Options: mixed, rotate, edge_cases, stratified
    ...
    # Component-ablation policies
    prompt_policy: str = "anchor"  # Options: anchor, no_anchor
    validation_policy: str = "full"  # Options: full, raw
    # Range-clip reference: by default clip generated values to the FULL training
    # range (majority + minority). When True, clip to the minority class only.
    # Tighter, but discards boundary-region samples that may aid classifier
    # discrimination. Default False to preserve historical results.
    clip_to_minority_class: bool = False
    # NOTE: default flipped from "multi_objective" to "generation_order" after the
    # 6-dataset, 18-run paired ablation (results/reviewer_revision/ablations/component_3seed)
    # showed no statistically significant performance gain (Holm-corrected Wilcoxon
    # p > 0.37 on F1, ROC-AUC, BalAcc, Precision, Recall) at substantial extra cost
    # (~1.8-2.1x wall-clock on larger datasets). Multi-objective selection is retained
    # as an opt-in module rather than the headline contribution; set
    # selection_policy="multi_objective" to re-enable it.
    selection_policy: str = "generation_order"  # Options: multi_objective, generation_order
...
class WorkflowResult:
    ...
    # Candidate-stage audit artifacts. These are populated for new reviewer
    # runs and are intentionally separate from the final selected samples.
    X_parsed: pd.DataFrame = field(default_factory=pd.DataFrame)
    X_validation_input: pd.DataFrame = field(default_factory=pd.DataFrame)
    X_accepted: pd.DataFrame = field(default_factory=pd.DataFrame)
    X_rejected: pd.DataFrame = field(default_factory=pd.DataFrame)
...
@dataclass
class NeutralFairnessAudit:
    """Placeholder audit used when fairness is disabled in the live workflow."""
    violations: List[Any] = field(default_factory=list)
    fairness_targets: List[Any] = field(default_factory=list)


@dataclass
class NeutralReauditResult:
    """Placeholder re-audit result used when fairness is disabled."""
...
class IterativeRefinementWorkflow:
    ...
    def __init__(self, config: Optional[WorkflowConfig] = None, method_name: str = "qualsynth", seed: Optional[int] = None, preprocessor=None, output_dir: Optional[str] = None):
        ...
        # Fairness is no longer part of the active QualSynth optimization path.
        if self.config.fairness_weight != 0.0:
            print("⚠️  Fairness objective disabled in active QualSynth path; resetting fairness_weight to 0.0")
        self.config.fairness_weight = 0.0
        ...
        if self.config.enable_adaptive_validation:
            ...
            self.adaptive_validator = AdaptiveValidator(
                ...
                adaptive_percentile_threshold=getattr(self.config, 'adaptive_percentile_threshold', 0.995),
            ...
            print(f"🔬 Adaptive Validator: ENABLED ({self.config.adaptive_std_threshold}σ, {getattr(self.config, 'adaptive_percentile_threshold', 0.995)*100:.1f}th percentile)")
    ...
    def run(
        ...
        all_parsed_samples = []
        all_validation_input_samples = []
        total_generated = 0
        total_validated = 0
        result = WorkflowResult()
        workflow_result = result
        result.prompt_policy = self.config.prompt_policy
        result.validation_policy = self.config.validation_policy
        result.selection_policy = self.config.selection_policy
        start_iteration = 1
        ...
        # ═══════════════════════════════════════════════════════════════════════════
        if self.output_dir and self.config.resume_generation:
            csv_output_dir = Path(self.output_dir) / 'logs'
            base_name = (
                f"{dataset_name}_{self.method_name}_seed{self.seed}"
                if self.seed is not None
                else f"{dataset_name}_{self.method_name}"
            )
            ...
            if existing_csv.exists():
                try:
                    ...
                    if len(existing_df) > 0:
                        ...
                        # Preserve full candidate lineage and usage when an
                        # explicitly authorized shortfall continuation resumes
                        # from an earlier finalized run.
                        parsed_path = csv_output_dir / f"{base_name}_parsed_candidates.csv"
                        validation_input_path = (
                            csv_output_dir / f"{base_name}_validation_input_candidates.csv"
                        )
                        if parsed_path.is_file():
                            parsed_df = pd.read_csv(parsed_path)
                            if "target" in parsed_df.columns:
                                parsed_df = parsed_df.drop(columns=["target"])
                            all_parsed_samples.append(parsed_df)
                            total_generated = len(parsed_df)
                        if validation_input_path.is_file():
                            validation_input_df = pd.read_csv(validation_input_path)
                            if "target" in validation_input_df.columns:
                                validation_input_df = validation_input_df.drop(
                                    columns=["target"]
                                )
                            all_validation_input_samples.append(validation_input_df)

                        requests_path = csv_output_dir / f"{base_name}_llm_requests.jsonl"
                        if requests_path.is_file():
                            ...
                            result.request_records.extend(prior_requests)
                            result.llm_calls = len(prior_requests)
                            ...
                            start_iteration = self._next_resume_iteration(
                                prior_requests
                            )
                        ...
                        print(
                            f"   🧾 Prior request records retained: {result.llm_calls}"
                        )
                        print(f"   🔢 Next scheduled iteration: {start_iteration}")
                        iteration_limit = (
                            f"up to {self.config.max_iterations} iterations"
                            if self.config.max_iterations > 0
                            else f"until target reached or stalled for {self.config.stall_iterations} iterations"
                        )
                        print(f"   ▶️  Starting generation loop ({iteration_limit})")
                        print()
        ...
        # STEP 2: Objective Setup
        print("STEP 2: Objective Setup")
        print("-"*80)
        initial_audit = NeutralFairnessAudit()
        print("Fairness objective: disabled")
        print("Objective weights: quality/performance + diversity only")
        print()
        ...
        print(f"Additional constraints: {len(schema_profile.fairness_constraints)}")
        print()
        ...
        print(f"Group-conditioned regions: {sum(len(regions) for regions in diversity_plan.group_sparse_regions.values())}")
        print()
        ...
        def register_no_progress() -> bool:
            """Count every completed no-progress iteration, including early exits."""
            nonlocal stall_counter
            current_count = (
                sum(len(frame) for frame in all_generated_samples)
                if all_generated_samples
                else 0
            )
            stall_counter, should_stop = self._advance_stall_counter(
                current_sample_count=current_count,
                last_sample_count=last_sample_count,
                stall_counter=stall_counter,
                stall_iterations=self.config.stall_iterations,
            )
            print(
                f"   ⚠️  No progress this iteration "
                f"(stall count: {stall_counter}/{self.config.stall_iterations})"
            )
            if should_stop:
                result.convergence_reason = (
                    f"Stalled: No progress for {stall_counter} iterations"
                )
                print(
                    f"   ❌ STALLED: No new samples for {stall_counter} "
        ...
        while True:
            ...
            iteration_data = {
                ...
                'prompt_policy': self.config.prompt_policy,
            ...
            generator = CounterfactualGenerator(
                ...
                strict_request_contract=self.config.strict_request_contract,
                ...
                n_few_shot_examples=self.config.n_few_shot_examples,
                dynamic_few_shot=self.config.dynamic_few_shot,
                few_shot_selection_strategy=self.config.few_shot_selection_strategy,
                prompt_policy=self.config.prompt_policy,
                raw_generation=self.config.validation_policy == "raw",
            )
            ...
            try:
                ...
                if generation_result.samples.empty:
                    ...
                    if register_no_progress():
                        break
                    continue
                
                X_gen_batch = generation_result.samples
                all_parsed_samples.append(X_gen_batch.copy())
                y_gen_batch = pd.Series([1] * len(X_gen_batch))  # Assume minority class
                
                # Log generation result
                print(f"   ✅ Generated: {len(X_gen_batch)} RAW samples")
                if len(X_gen_batch) > 0 and self.config.validation_policy != "raw":
                    print(f"   📊 Columns: {len(X_gen_batch.columns)}")
                sys.stdout.flush()
                
                # Post-processing: Clip numerical values to training data range.
                # When clip_to_minority_class=True, restrict bounds to minority-class
                # rows only (tighter support, removes boundary-region samples).
                if len(X_gen_batch) > 0 and self.config.validation_policy != "raw":
                    if getattr(self.config, "clip_to_minority_class", False):
                        clip_ref = X_train[y_train == 1]
                        clip_label = "minority-class"
                    else:
                        clip_ref = X_train
                        clip_label = "training"
                    n_clipped_total = 0
                    for col in X_gen_batch.columns:
                        if col in clip_ref.columns:
                            # Only clip numerical columns
                            if pd.api.types.is_numeric_dtype(clip_ref[col]):
                                ref_col = clip_ref[col]
                                if ref_col.empty:
                                    continue
                                col_min = ref_col.min()
                                col_max = ref_col.max()
                    ...
                    if n_clipped_total > 0:
                        print(f"   ✂️  Clipped {n_clipped_total} out-of-range values to {clip_label} bounds")
                
                if hasattr(generation_result, 'llm_calls'):
                    print(f"   📞 LLM calls: {generation_result.llm_calls}")
                if hasattr(generation_result, 'total_tokens') and generation_result.total_tokens > 0:
                    print(f"   🎫 Tokens: {generation_result.total_tokens}")
                iteration_data['llm_calls'] = getattr(generation_result, 'llm_calls', 0)
                iteration_data['total_tokens'] = getattr(generation_result, 'total_tokens', 0)
                iteration_data['prompt_tokens'] = getattr(generation_result, 'prompt_tokens', 0)
                iteration_data['completion_tokens'] = getattr(generation_result, 'completion_tokens', 0)
                iteration_data['reasoning_tokens'] = getattr(generation_result, 'reasoning_tokens', 0)
                iteration_data['generation_cost'] = getattr(generation_result, 'cost', 0.0)
                iteration_data['request_records'] = list(
                    getattr(generation_result, 'metadata', {}).get('request_records', [])
                )
                workflow_result.llm_calls += iteration_data['llm_calls']
                workflow_result.total_tokens += iteration_data['total_tokens']
                workflow_result.prompt_tokens += iteration_data['prompt_tokens']
                workflow_result.completion_tokens += iteration_data['completion_tokens']
                workflow_result.reasoning_tokens += iteration_data['reasoning_tokens']
                workflow_result.total_cost += iteration_data['generation_cost']
                workflow_result.request_records.extend(iteration_data['request_records'])
            ...
            if self.config.validation_policy == "raw":
            ...
            if (
                self.config.validation_policy != "raw"
                and X_gen_encoded is not None
                and len(X_gen_encoded) > 0
            ):
            ...
            # Freeze the actual validator input after encoding, clipping, and
            # deterministic missing-value repair. This makes accepted and
            # rejected artifacts mutually exclusive at the validation stage.
            all_validation_input_samples.append(X_gen_encoded.copy())
            ...
            validation_mode = getattr(self.config, "validation_mode", "standard").lower()
            if self.config.validation_policy == "raw":
                print("   ⚠️  Raw validation policy enabled - bypassing all validators")
                X_valid = X_gen_encoded.copy()
                y_valid = pd.Series([1] * len(X_valid))
                print(f"   ✅ Raw rows accepted for downstream stress test: {len(X_valid)}")
                if len(X_valid) == 0:
                    print("⚠️  No raw samples parsed in this iteration")
                    if register_no_progress():
                        break
                    continue

            elif validation_mode in {"standard", "auto", "high_dimensional"}:
                print(f"   📋 Using Universal Validator ({validation_mode} mode)")
                ...
                try:
                    from ..validation.universal_validator import UniversalValidator
                except ImportError:
                    from src.qualsynth.validation.universal_validator import UniversalValidator
                ...
                universal_validator = UniversalValidator(
                    ...
                    fairness_weight=0.0,
                    performance_weight=self.config.performance_weight,
                    max_samples=len(X_gen_encoded),
                    verbose=True,
                    statistical_std_threshold=self.config.adaptive_std_threshold,
                    enable_semantic_dedup=self.config.enable_semantic_dedup,
                    validation_mode=validation_mode,
                )
                universal_result = universal_validator.validate_and_select(
                ...
                X_valid = universal_result.X_validated
                y_valid = universal_result.y_validated

                print(f"   ✅ Universal validation complete:")
                print(f"      Original: {universal_result.n_original}")
                print(f"      After dedup: {universal_result.n_after_dedup} ({universal_result.duplicate_ratio*100:.1f}% duplicates)")
                print(f"      After quality: {universal_result.n_after_quality} ({universal_result.quality_pass_rate*100:.1f}% pass)")
                print(f"      Final selected: {universal_result.n_after_selection} ({universal_result.overall_pass_rate*100:.1f}% overall)")

                if len(X_valid) == 0:
                    print("⚠️  No valid samples after universal validation")
                    print("   ⚠️  Continuing to next iteration...")
                    if register_no_progress():
                        break
                    continue

            elif self.config.enable_adaptive_validation and self.adaptive_validator is not None:
                ...
                if not valid_sample_list or len(valid_sample_list) == 0:
                    ...
                    if len(validation_report.results) > 0:
                        ...
                        for i, res in enumerate(validation_report.results[:5]):
                            if not res.is_valid:
                                print(
                                    f"  Sample {i}: errors={res.errors}, warnings={res.warnings}, "
                                    f"schema_valid={res.schema_valid}, duplicate_valid={res.duplicate_valid}"
                                )
                    print("   ⚠️  Continuing to next iteration...")
                    if register_no_progress():
                        break
            ...
            if self.config.validation_policy == "raw":
                print("   ⚠️  Raw validation policy enabled - duplicate prevention bypassed")
                ...
                filter_stats = {
                    ...
                    'duplicate_rate': float(X_valid.duplicated().mean()) if len(X_valid) else 0.0,
                    'diversity_score': 0.0,
                    'constant_columns': []
                }
            elif self.config.enable_sota_dedup and self.sota_dedup is not None:
            ...
            print("-"*80)
            current_selected = sum(len(frame) for frame in all_generated_samples)
            selection_quota = self._selection_quota(
                target_samples=self.config.target_samples,
                current_selected=current_selected,
                batch_size=self.config.batch_size,
                valid_candidates=len(X_valid),
            )
            print(f"   🎯 Selecting best {selection_quota} from {len(X_valid)} valid samples...")
            print(f"   ⚖️  Weights: Quality={self.config.performance_weight:.2f}, "
                  f"Diversity={self.config.diversity_weight:.2f}")
            if self.config.selection_policy == "generation_order" or self.config.validation_policy == "raw":
                X_selected = X_valid.head(selection_quota).copy()
                y_selected = pd.Series([1] * len(X_selected))
                print("   ⚠️  Multi-objective optimizer bypassed")
                print(f"   ✅ Selected by generation order: {len(X_selected)} samples")
            else:
                optimization_result = self.optimizer.optimize(
                    candidates_df=X_valid,
                    n_samples=selection_quota,
            ...
            # 5e. Objective Progress Summary
            print(f"Step {iteration}E: Progress Summary")
            print("-"*80)
            ...
            print(f"   ⏳ Fairness objective disabled; skipping re-audit")
            reaudit_result = NeutralReauditResult()
        ...
        # Finalize stage-level candidate artifacts even when no final row was
        # selected. This keeps failed pilots auditable.
        result.X_parsed = (
            pd.concat(all_parsed_samples, axis=0, ignore_index=True)
            if all_parsed_samples
            else pd.DataFrame(columns=X_train.columns)
        )
        result.X_validation_input = (
            pd.concat(all_validation_input_samples, axis=0, ignore_index=True)
            if all_validation_input_samples
            else pd.DataFrame(columns=X_train.columns)
        )
        result.X_accepted = (
            pd.concat(all_validated_samples, axis=0, ignore_index=True)
            if all_validated_samples
            else pd.DataFrame(columns=X_train.columns)
        )
        result.X_rejected = self._row_multiset_difference(
            result.X_validation_input,
            result.X_accepted,
        )
        ...
        if all_generated_samples:
            ...
            result.total_iterations = len(result.iteration_history)
    ...
    @staticmethod
    def _selection_quota(
        target_samples: int,
        current_selected: int,
        batch_size: int,
        valid_candidates: int,
    ) -> int:
        """Return a non-negative quota that cannot overshoot the target."""
        if target_samples < 0 or current_selected < 0 or batch_size < 0 or valid_candidates < 0:
            raise ValueError("Selection counts must be non-negative")
        remaining = max(0, target_samples - current_selected)
        return min(remaining, batch_size, valid_candidates)

    @staticmethod
    def _advance_stall_counter(
        current_sample_count: int,
        last_sample_count: int,
        stall_counter: int,
        stall_iterations: int,
    ) -> Tuple[int, bool]:
        """Advance/reset stall state and report whether the limit is reached."""
        if stall_iterations < 1:
            raise ValueError("stall_iterations must be at least 1")
        if current_sample_count > last_sample_count:
            return 0, False
        next_counter = stall_counter + 1
        return next_counter, next_counter >= stall_iterations

    @staticmethod
    def _next_resume_iteration(request_records: List[Dict[str, Any]]) -> int:
        """Continue iteration-dependent sampling after retained primary attempts."""
        completed_iterations = sum(
            int(record.get("attempt", 1) or 1) == 1
            and int(record.get("batch_index", 0) or 0) == 0
            for record in request_records
        )
        return completed_iterations + 1

    @staticmethod
    def _row_multiset_difference(
        candidates: pd.DataFrame,
        accepted: pd.DataFrame,
    ) -> pd.DataFrame:
        """Return candidate rows not represented in the accepted row multiset."""
        from collections import Counter

        if candidates is None or candidates.empty:
            columns = candidates.columns if candidates is not None else []
            return pd.DataFrame(columns=columns)
        common_columns = [column for column in candidates.columns if column in accepted.columns]
        if not common_columns:
            return candidates.copy().reset_index(drop=True)
        candidate_view = candidates[common_columns].reset_index(drop=True)
        accepted_view = accepted[common_columns].reset_index(drop=True)
        candidate_hashes = pd.util.hash_pandas_object(candidate_view, index=False).astype(str)
        accepted_counts = Counter(
            pd.util.hash_pandas_object(accepted_view, index=False).astype(str).tolist()
        )
        rejected_positions = []
        for position, row_hash in enumerate(candidate_hashes.tolist()):
            if accepted_counts[row_hash] > 0:
                accepted_counts[row_hash] -= 1
            else:
                rejected_positions.append(position)
        return candidates.iloc[rejected_positions].copy().reset_index(drop=True)
    ...
    def _save_iteration_csv(
        ...
        try:
            ...
            if hasattr(self, 'preprocessor') and self.preprocessor is not None:
                encoded_validator_output = self.config.validation_policy != "raw"
                selected_df_raw = decode_features(
                    selected_df,
                    self.preprocessor,
                    assume_encoded=encoded_validator_output,
                )
                validated_df_raw = decode_features(
                    validated_df,
                    self.preprocessor,
                    assume_encoded=encoded_validator_output,
                )
            ...
            # Enforce exact row-level uniqueness in saved artifacts.
            generated_before = len(generated_df)
            generated_df = generated_df.drop_duplicates(ignore_index=True)
            generated_removed = generated_before - len(generated_df)
            if generated_removed > 0:
                print(f"   ℹ️  Removed {generated_removed} exact duplicate generated rows before saving")
            ...
            validated_before = len(validated_samples_df)
            validated_samples_df = validated_samples_df.drop_duplicates(ignore_index=True)
            validated_removed = validated_before - len(validated_samples_df)
            if validated_removed > 0:
                print(f"   ℹ️  Removed {validated_removed} exact duplicate validated rows before saving")
