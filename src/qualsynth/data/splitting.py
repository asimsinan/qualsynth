    ...
    if return_raw and 'preprocessor' in split_data:
        ...
        for key in ['X_train', 'X_val', 'X_test']:
            if key in split_data:
                # Persisted split features are always encoded. Do not infer their
                # representation from a sample-dependent value range: on Thyroid,
                # the extreme age observation made that heuristic vary by seed.
                split_data_raw[key] = decode_features(
                    split_data[key].copy(),
                    preprocessor,
                    assume_encoded=True,
                )
...
def decode_features(
    ...
    Args:
        ...
        assume_encoded: Explicitly identify known encoded (``True``) or raw
            (``False``) input. ``None`` retains conservative auto-detection for
            external callers.
    ...
    if assume_encoded is None and hasattr(preprocessor, 'label_encoders') and preprocessor.label_encoders:
    ...
    if assume_encoded is not False and hasattr(preprocessor, 'numerical_features') and preprocessor.numerical_features:
        num_cols = [c for c in preprocessor.numerical_features if c in X.columns]
        if num_cols:
            if assume_encoded is True:
    ...
    if assume_encoded is not False and hasattr(preprocessor, 'label_encoders'):
    ...
    if hasattr(preprocessor, 'label_encoders'):
        for col, encoder in preprocessor.label_encoders.items():
            if col in X_encoded.columns:
                # Match against the encoder's native class dtype. Converting every
                # value to text breaks numeric categorical encoders (for example,
                # Thyroid's 0/1 indicator columns): ``"1"`` is not equal to the
                # integer class ``1`` and previously fell back to the first class.
                def safe_encode(x, enc=encoder):
                    classes = np.asarray(enc.classes_)
                    if pd.isna(x):
                        matched_class = classes[0]
                    else:
                        candidate = x.strip() if isinstance(x, str) else x
                        exact_matches = np.flatnonzero(classes == candidate)

                        if exact_matches.size:
                            matched_class = classes[exact_matches[0]]
                        elif np.issubdtype(classes.dtype, np.number):
                            numeric_candidate = pd.to_numeric(candidate, errors='coerce')
                            if pd.notna(numeric_candidate):
                                numeric_matches = np.flatnonzero(
                                    np.isclose(
                                ...
                                numeric_matches = np.array([], dtype=int)
                            matched_class = (
                                classes[numeric_matches[0]]
                                if numeric_matches.size
                                else classes[0]
                            )
                        else:
                            normalized_candidate = str(candidate).strip()
                            normalized_classes = np.asarray(
                                [str(value).strip() for value in classes]
                            )
                            text_matches = np.flatnonzero(
                                normalized_classes == normalized_candidate
                            )
                            matched_class = (
                                classes[text_matches[0]]
                                if text_matches.size
                                else classes[0]
                            )

                    return enc.transform([matched_class])[0]
