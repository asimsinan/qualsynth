...
class SchemaProfiler:
    ...
    def _analyze_feature(
        ...
        is_integer_numeric = False
        if is_numeric:
            try:
                non_null = series.dropna()
                is_integer_numeric = bool(non_null.apply(lambda x: float(x).is_integer()).all())
        ...
        elif is_numeric and not is_integer_numeric:
            # Continuous scientific measurements can have few unique values in
            # small folds; do not coerce them to categorical solely by count.
